...
<div class="pageContent">
    <form id="BK200302Form" method="post" action="BK2003/approve.do" class="pageForm -validate" onsubmit="return validateCallback(this, navTabAjaxDone);">
...
<script type="text/javascript">
    // 初始化下拉菜单、单选按钮
    $("#BK200302Form").find("#gender").val("${model.gender}");
    $("#BK200302Form").find("input:radio[value=${model.marriage}]").attr("checked",true);
    // 审批意见列表
    $("#BK200302Form").find("#commentList").ajaxUrl({
    	url:"BK6001/commentList.do",
    	data:{taskId:$("#BK200302Form").find("#taskId").val()}
    });
    // 表单项目编辑权限控制
    if("${nodeName}" != "客户经理填写贷款报告"){
    	$("#BK200302Form").find(".form").find("*").attr("disabled","disabled");
    	$("#BK200302Form").find("#commentText").removeAttr("disabled");
    }
    if("${nodeName}" == "放款员审核放款手续" || "${nodeName}" == "财务放款"){
        $("#BK200302Form").find("#withdraw").hide();
        $("#BK200302Form").find("#abort").hide();
    }
    // 退回
    function withdraw(){
    	$("#BK200302Form").attr("action", "BK2003/withdraw.do");
    	$("#BK200302Form").submit();
    }
    // 终止
    function abort(){
        $("#BK200302Form").attr("action", "BK2003/abort.do");
        $("#BK200302Form").submit();
    }
</script>