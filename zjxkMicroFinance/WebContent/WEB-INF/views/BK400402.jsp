							...
							<tbody id="srcTbody">
								<c:forEach var="fmodule"  items="${model}" varStatus="status">
									<tr  style="cursor: pointer;" class="srcTr" id="${fmodule.modname}" onclick="addClk(this)">
									    <td class="tbodyTd" style="width:1%;height:20px"><span style="display:none;" ><input id="${fmodule.id}" name="chkSel"  type="checkbox" value=""></span></td>
	...
	function addModule(){
		$('.srcTr').each(function(){
			...
			if (chk == true) {
				...
				var $spanTd1Chk = $("<input id="+obj.attr('id')+" name='chkSel' type='checkbox'/>");
				$("#tarTbody").append($tr);
				$tr.append($td);
				$td.append($spanTd1);
