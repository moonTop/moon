...
<form id="pagerForm" method="post" action="BK0402/list.do">
...
<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="BK0402/list.do" method="post">
    ...
    <table id="gridTable" class="table" width="100%" layoutH="194">
        ...
        <c:forEach var="mlog" items="${page.resultList}" varStatus="status">
            <tr target="id" rel="${mlog.id}">
                ...
                <td>${mlog.modulename}</td>
                <td>${mlog.opstyle}</td>
                <td>${mlog.opcontent}</td>
                <td>${mlog.opperson}</td>
                <td>${mlog.formattedOptime}</td>
                <td>${mlog.ip}</td>
            </tr>
        </c:forEach>
        </tbody>
