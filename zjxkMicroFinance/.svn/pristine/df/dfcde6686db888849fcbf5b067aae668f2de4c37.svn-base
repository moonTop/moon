...
<div class="pageContent">
    <form id="BK200303Form" method="post">
        <input type="hidden" id="taskId" name="taskId" value="${taskId}" />
        <div class="pageFormContent" layoutH="56">
            <div class="titleBar">
                <div class="navTitle">楼易贷申请单/</div><div class="pageTitle">查看</div>
            </div>
...
<script type="text/javascript">
    // 初始化下拉菜单、单选按钮
    $("#BK200303Form").find("#gender").val("${model.gender}");
    $("#BK200303Form").find("input:radio[value=${model.marriage}]").attr("checked",true);
    // 全部不可编辑
    $("#BK200303Form").find("*").attr("disabled","disabled");
    // 审批意见列表
    $("#BK200303Form").find("#commentList").ajaxUrl({
        url:"BK6001/commentList.do",
        data:{taskId:$("#BK200303Form").find("#taskId").val()}
    });
</script>