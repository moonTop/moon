...
public class Browser extends HttpServlet {
	...
	private void setVersion() {
		...
		if (this.company == "Microsoft") {
			String str = this.userAgent.substring(this.userAgent
					.indexOf("msie") + 5);
			this.version = str.substring(0, str.indexOf(";"));
		} else {
			tmpString = (this.userAgent.substring(
					tmpPos = (this.userAgent.indexOf("/")) + 1, tmpPos
	...
	private void setMinorVersion() {
		this.minorVersion = this.version.substring(
				this.version.indexOf(".") + 1).trim();
	}
	...
	private void setLanguage() {
		...
		if (prefLanguage != null) {
			...
			for (int idx = 0; idx < elements; idx++) {
				if (this.supportedLanguages.containsKey((language = st
						.nextToken()))) {
	...
	/**
	 * Liefert das L?nderkürzel der vom Benutzer bevorzugten Sprache.
	 */
