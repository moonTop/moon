			...
			<div data-role="content">
				...
				<div class="ui-bar ui-bar-a">
					<div data-role="controlgroup" data-type="horizontal"
						...
						<c:if test="${nodeName!='放款员审核放款手续' && nodeName!='财务放款'}">
						<a href="#" data-role="button" data-inline="true" id="withdraw">退回</a> 
						<a href="#" data-role="button" data-inline="true" id="abort">否决</a>
						</c:if>
						<a href="#"	data-role="button" data-inline="true" id="passId">通过</a>						
					</div>
				</div>
				<div >
					<table width="100%" border="1" class="approvalForm" cellspacing="0"
...
$("div[data-role='page']").live('pageshow',function(event,ui){
		//表单提交  
    $("#passId").bind("click", function() {
    	var url= "${contextPath}";
         $.ajax({
        	 type: "POST",
        	 url: "${contextPath}/views/mobileManage/approve.do",
        	 data: $("#approveForm01").serialize(),
        	 dataType:"json",
        	 success: function(data){
        		 	alert("您的审批操作，已经成功执行！");
                    $.mobile.changePage(url +"/views/mobileManage/todoList.do?","slideleft", false, false);  
             }
        });   
    });  
    ...
    $("#withdraw").bind("click", function() {
    	var url= "${contextPath}";
         $.ajax({
        	 type: "POST",
        	 url: "${contextPath}/views/mobileManage/withdraw.do",
        	 data: $("#approveForm01").serialize(),
        	 dataType:"json",
        	 success: function(data){
        		 	alert("您的退回操作，已经成功执行！");
    ...
    $("#abort").bind("click", function() {
    	var url= "${contextPath}";
         $.ajax({
        	 type: "POST",
        	 url: "${contextPath}/views/mobileManage/abort.do",
        	 data: $("#approveForm01").serialize(),
        	 dataType:"json",
        	 success: function(data){
        		 	alert("否决操作，已经成功执行！");
