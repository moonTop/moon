...
<form id="pagerForm" method="post" action="BK1003/houseList.do">
...
<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="BK1003/houseList.do" method="post">
...
<div class="pageContent">
    <div class="panelBar">
        <ul class="toolBar">
            <li><a class="edit" href="BK1003/edit/{id}.do" title="修改用户" target="dialog" width="600" height="400" rel="BK100301"><span>修改</span></a></li>
        </ul>
    </div>
    <table id="gridTable" class="table" width="100%" layoutH="194">
        <thead>
            <tr>
				...
				<th style="font-weight:bold; text-align: center;" width="80" style="font-weight:bold;">手机号码</th>
                <th style="font-weight:bold; text-align: center;" width="80" style="font-weight:bold;">贷款金额</th>
                <th style="font-weight:bold; text-align: center;" width="150" style="font-weight:bold;">是否有房屋贷款</th>
        ...
        <tbody>
        <c:forEach var="house" items="${page.resultList}" varStatus="status">
            <tr target="id" rel="${house.id}">
                <td align="center">${status.index+1}</td>
                <td align="center">${house.name}</td>
                <td align="center">${house.phone}</td>
                <td align="center">${house.loanamount}</td>
                <td align="center">${house.houseloanflg}</td>
                <td align="center"><fmt:formatDate value='${house.applydate}' pattern='yyyy-MM-dd' /></td>
                <td align="center">
                	<c:choose>
                		<c:when test="${house.status == 0}">未处理</c:when>
