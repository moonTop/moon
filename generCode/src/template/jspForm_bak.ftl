...
<script type="text/javascript">
	...
	var submitNow = function($dialog, $grid, $pjq) {
		...
		$.post(url, tw.serializeObject($('form')), function(result) {
			parent.tw.progressBar('close');//关闭上传进度条
	...
	$(function() {
		...
		if ($(':input[name="id"]').val().length > 0) {
			...
			$.post(tw.contextPath + '/${beanNameLower}/getById.htm', {
				...
				if (result.id != undefined) {
					...
					/* 照片
					if (result.photo) {
						$('#photo').attr('src', tw.contextPath + result.photo);
					}*/
				}
		...
		uploader = new plupload.Uploader({//上传插件定义
			...
			flash_swf_url : tw.contextPath + '/jslib/plupload_1_5_7/plupload/js/plupload.flash.swf',// Flash环境路径设置
			url : tw.contextPath + '/plupload?fileFolder=/userPhoto',//上传文件路径
		...
		uploader.bind('BeforeUpload', function(uploader, file) {//上传之前
			...
			$('.ext-icon-arrow-cross').hide();
		});
		uploader.bind('FilesAdded', function(up, files) {//选择文件后
			$.each(files, function(i, file) {
				$('#filelist').append('<div id="' + file.id + '">' + file.name + '(' + plupload.formatSize(file.size) + ')<strong></strong>' + '<span onclick="uploader.removeFile(uploader.getFile($(this).parent().attr(\'id\')));$(this).parent().remove();" style="cursor:pointer;" class="ext-icon-arrow-cross" title="删除">&nbsp;&nbsp;&nbsp;&nbsp;</span></div>');
			});
			up.refresh();
		});
		uploader.bind('UploadProgress', function(up, file) {//上传进度改变
			...
			parent.tw.progressBar({//显示文件上传滚动条
				title : '文件上传中...',
				value : msg
			});
		});
		...
		<fieldset>
			...
			<table class="table" style="width: 100%;">
				...
				<tr>
					<th>照片上传</th>
					<td><div id="container">
							<a id="pickfiles" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-zoom-zoom'">选择文件</a>
